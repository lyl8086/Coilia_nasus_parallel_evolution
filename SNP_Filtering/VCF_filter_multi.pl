...
if ($sort) {
    ...
    if (substr($outfile, -2) eq 'gz') {
        # use bgzip.
        `(zgrep '^#' $outfile; zgrep -v '^#' $outfile | sort -k1,1V -k2,2n ) | bgzip --threads 4 -c >$outfile.1 && mv $outfile.1 $outfile`;
    } else {
        `(grep '^#' $outfile; grep -v '^#' $outfile | sort -k1,1V -k2,2n) >$outfile.1 && mv $outfile.1 $outfile`;
    }
    print STDERR "done.\n";
}
open($out_fh, ">>$outfile.log") or die "$!";
