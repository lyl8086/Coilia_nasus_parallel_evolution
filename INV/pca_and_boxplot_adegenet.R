...
for (i in 1:length(file_names)) {
	...
	pca <- glPca(gldf, nf=2, loadings=F, useC=F, parallel=F)
	pc  <- pca$scores
	pc.va <- pca$eig
	pc1 <- pc[,1]
...
pdf("LDna.PCA.adegenet.pdf", width=14, height=a*3)
ggarrange(plotlist=plots1, ncol = 3, nrow=a) 
dev.off()

pdf("LDna.het.adegenet.pdf", width=14, height=a*3)
