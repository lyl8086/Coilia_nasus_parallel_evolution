...
for (i in 1:length(file_names)) {
	...
	pc  <- snpgdsPCA(gds, eigen.cnt=length(seqGetData(gds, "sample.id")),
		autosome.only=F, remove.monosnp=F, algorithm="exact", verbose=FALSE)
	pc.va <- pc$varprop*100
	set.seed(222) # for reproducible
	# partinate to 3 clusters by kmeans.
	pc1 <- pc$eigenvect[,1]
	names(pc1) <- pc$sample.id
...
pdf("LDna.PCA.SNPRelate.pdf", width=14, height=a*3)
ggarrange(plotlist=plots1, ncol = 3, nrow=a) 
dev.off()

pdf("LDna.het.SNPRelate.pdf", width=14, height=a*3)
